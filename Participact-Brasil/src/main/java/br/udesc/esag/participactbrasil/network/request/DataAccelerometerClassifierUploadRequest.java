...
public class DataAccelerometerClassifierUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataAccelerometerClassifierUploadRequest.class);

    private final static String RELATIVE_URL = "accelerometerClassifier";
    private final static String TABLE = PipelineAccelerometerClassifier.TBL_ACCELEROMETER_CLASSIFIER;

    String key;
    List<ContentValues> data;

    public DataAccelerometerClassifierUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataAccelerometerClassifierProtoList(data).toByteArray());
