...
public class DataActivityRecognitionCompareUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataActivityRecognitionCompareUploadRequest.class);

    private final static String RELATIVE_URL = "activityrecognitioncompare";
    private final static String TABLE = PipelineActivityRecognitionCompare.TBL_ACTIVITY_RECOGNITION_COMPARE;

    String key;
    List<ContentValues> data;

    public DataActivityRecognitionCompareUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataActivityRecognitionComparatorProtoList(data).toByteArray());
