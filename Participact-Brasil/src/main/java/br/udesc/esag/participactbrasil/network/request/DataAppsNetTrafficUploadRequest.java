...
public class DataAppsNetTrafficUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataAppsNetTrafficUploadRequest.class);

    private final static String RELATIVE_URL = "appsnettraffic";
    private final static String TABLE = PipelineAppsNetTraffic.TBL_NET_TRAFFIC_APPS;

    String key;
    List<ContentValues> data;

    public DataAppsNetTrafficUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataAppsNetTrafficProto(data).toByteArray());
