...
public class DataBatteryUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private final static String RELATIVE_URL = "battery";
    private final static String TABLE = PipelineBattery.TBL_BATTERY;

    private static final Logger logger = LoggerFactory.getLogger(DataBatteryUploadRequest.class);

    String key;
    List<ContentValues> data;

    public DataBatteryUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataBatteryProtoList(data).toByteArray());
