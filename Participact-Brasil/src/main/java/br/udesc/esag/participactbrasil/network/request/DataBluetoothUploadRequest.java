...
public class DataBluetoothUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataBluetoothUploadRequest.class);

    private final static String RELATIVE_URL = "bluetooth";
    private final static String TABLE = PipelineBluetooth.TBL_BLUETOOH;

    String key;
    List<ContentValues> data;

    public DataBluetoothUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataBluetoothProto(data).toByteArray());
