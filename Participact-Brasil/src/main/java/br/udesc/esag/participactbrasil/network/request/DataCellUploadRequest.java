...
public class DataCellUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataCellUploadRequest.class);

    private final static String RELATIVE_URL = "cell";
    private final static String TABLE = PipelineCell.TBL_CELL;

    String key;
    List<ContentValues> data;

    public DataCellUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataCellProto(data).toByteArray());
