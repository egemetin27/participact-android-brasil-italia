...
public class DataConnectionTypeUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataConnectionTypeUploadRequest.class);

    private final static String RELATIVE_URL = "connectiontype";
    private final static String TABLE = PipelineConnectionType.TBL_CONNECTION_TYPE;

    String key;
    List<ContentValues> data;

    public DataConnectionTypeUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataConnectionTypeProto(data).toByteArray());
