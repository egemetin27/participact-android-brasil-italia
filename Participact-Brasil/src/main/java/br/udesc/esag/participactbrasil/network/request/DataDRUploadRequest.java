...
public class DataDRUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataDRUploadRequest.class);

    private final static String RELATIVE_URL = "dr";
    private final static String TABLE = PipelineDR.TBL_DR;

    String key;
    List<ContentValues> data;

    public DataDRUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataDRProtoList(data).toByteArray());
