...
public class DataDeviceNetTrafficUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataDeviceNetTrafficUploadRequest.class);

    private final static String RELATIVE_URL = "devicenettraffic";
    private final static String TABLE = PipelineDeviceNetTraffic.TBL_NET_TRAFFIC_DEVICE;

    String key;
    List<ContentValues> data;

    public DataDeviceNetTrafficUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataDeviceNetTrafficProto(data).toByteArray());
