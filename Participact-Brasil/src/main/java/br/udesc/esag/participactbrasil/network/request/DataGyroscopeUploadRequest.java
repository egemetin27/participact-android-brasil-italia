...
public class DataGyroscopeUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataGyroscopeUploadRequest.class);

    private final static String RELATIVE_URL = "gyroscope";
    private final static String TABLE = PipelineGyroscope.TBL_GYROSCOPE;

    String key;
    List<ContentValues> data;

    public DataGyroscopeUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataGyroscopeProto(data).toByteArray());
