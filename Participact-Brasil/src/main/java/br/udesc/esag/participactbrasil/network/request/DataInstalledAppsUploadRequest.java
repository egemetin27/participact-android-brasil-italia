...
public class DataInstalledAppsUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataInstalledAppsUploadRequest.class);

    private final static String RELATIVE_URL = "installedapps";
    private final static String TABLE = PipelineInstalledApps.TBL_INSTALLED_APPS;

    String key;
    List<ContentValues> data;

    public DataInstalledAppsUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataInstalledAppsProto(data).toByteArray());
