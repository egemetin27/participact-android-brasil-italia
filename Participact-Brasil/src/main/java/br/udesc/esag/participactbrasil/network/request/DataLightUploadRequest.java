...
public class DataLightUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataLightUploadRequest.class);

    private final static String RELATIVE_URL = "light";
    private final static String TABLE = PipelineLight.TBL_LIGHT;

    String key;
    List<ContentValues> data;

    public DataLightUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataLightProto(data).toByteArray());
