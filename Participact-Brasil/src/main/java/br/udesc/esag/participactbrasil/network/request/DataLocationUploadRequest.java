...
public class DataLocationUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataLocationUploadRequest.class);

    private final static String RELATIVE_URL = "location";
    private final static String TABLE = PipelineLocation.TBL_LOCATION;

    String key;
    List<ContentValues> data;

    public DataLocationUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataLocationProtoList(data).toByteArray());
