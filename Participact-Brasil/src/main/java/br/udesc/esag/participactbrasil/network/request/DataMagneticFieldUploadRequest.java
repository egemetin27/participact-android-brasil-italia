...
public class DataMagneticFieldUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataMagneticFieldUploadRequest.class);

    private final static String RELATIVE_URL = "magneticfield";
    private final static String TABLE = PipelineMagneticField.TBL_MAGNETIC_FIELD;

    String key;
    List<ContentValues> data;

    public DataMagneticFieldUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataMagneticFieldProto(data).toByteArray());
