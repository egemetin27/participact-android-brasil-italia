...
public class DataPhoneCallEventUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataPhoneCallEventUploadRequest.class);

    private final static String RELATIVE_URL = "phonecallevent";
    private final static String TABLE = PipelinePhoneCallEvent.TBL_PHONE_CALL_EVENT;

    String key;
    List<ContentValues> data;

    public DataPhoneCallEventUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataPhoneCallEventProto(data).toByteArray());
