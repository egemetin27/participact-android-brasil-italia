...
public class DataSystemStatsUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataSystemStatsUploadRequest.class);

    private final static String RELATIVE_URL = "systemstats";
    private final static String TABLE = PipelineSystemStats.TBL_SYSTEM_STATS;

    String key;
    List<ContentValues> data;

    public DataSystemStatsUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataSystemStatsProto(data).toByteArray());
