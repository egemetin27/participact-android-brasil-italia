...
public class DataWifiScanUploadRequest extends ApacheHttpSpiceRequest<ResponseMessage> {

    private static final Logger logger = LoggerFactory.getLogger(DataWifiScanUploadRequest.class);

    private final static String RELATIVE_URL = "wifiscan";
    private final static String TABLE = PipelineWifiScan.TBL_WIFI_SCAN;

    String key;
    List<ContentValues> data;

    public DataWifiScanUploadRequest(Context context, List<ContentValues> data) {
    ...
    public ResponseMessage loadDataFromNetwork() throws Exception {
        ...
        ByteArrayEntity entity = new ByteArrayEntity(ContentValuesToProto.convertToDataWifiScanProto(data).toByteArray());
