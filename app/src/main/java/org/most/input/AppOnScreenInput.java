...
public class AppOnScreenInput extends PeriodicInput {
    ...
    public void workToDo() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            Log.d("AppsOnScreen", "Start >>>>>>>>>>>>>>>>>>>>>>>>>>>");
            List<AndroidAppProcess> processes = AndroidProcesses.getRunningAppProcesses();
            for (AndroidAppProcess process : processes) {
                if (process.foreground) {
                    try {
                        PackageInfo packageInfo = process.getPackageInfo(getContext(), 0);
                        String appName = packageInfo.applicationInfo.loadLabel(getContext().getPackageManager()).toString();
                        Log.d("AppsOnScreen", "Package: " + process.getPackageName() + " Name: " + process.name + " appName: " + appName);

                        DataBundle b = _bundlePool.borrowBundle();
                        b.putString(KEY_APPONSCREEN, process.getPackageName());
                        b.putString(KEY_APPONSCREENACTIVITY, appName);
		 ...
		 */
            if (fgApp != null && fgAct != null) {
                String packageName = fgApp.baseActivity.getPackageName();
                String activity = fgAct.getClassName();
                Log.d("AppsOnScreen", "Package: " + packageName + ", Activity: " + activity);
                DataBundle b = _bundlePool.borrowBundle();
                b.putString(KEY_APPONSCREEN, packageName);
                b.putString(KEY_APPONSCREENACTIVITY, activity);
                b.putLong(Input.KEY_TIMESTAMP, System.currentTimeMillis());
                b.putInt(Input.KEY_TYPE, Input.Type.APPONSCREEN.toInt());
                post(b);
            }
